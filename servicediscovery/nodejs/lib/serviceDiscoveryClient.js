...
ServiceDiscoveryClient.prototype.ls = function ls(path, cb) {
    ...
    zk.once('connected', function (err) {
        if (err) throw err;
        console.log("zk session established, id=%s", zk.getSessionId());
        zk.exists(path, function(error, path) {
            if (error) {
                console.log("zk error: %s, %s", error, path);
                cb(error, path);
            } else {
                if (path){
                    ...
                    console.log("exists node error: %s, stat: %j", error, path);
                    process.nextTick(function() {
                        zk.close();
                    });
...
ServiceDiscoveryClient.prototype.getApplications = function getApplications(cb) {
    ...
    zk.once('connected',function(err) {
        ...
        zk.getChildren(path, false, function(err, children) {
            if (err) {
                rc = err.getCode()
                console.log("zk error: %d, %s, %s", rc, err, path);
                if (rc === -101) {
                ...
                console.log("children for node: %s - %j", path, children);
                cb(null, children);
            }
            process.nextTick(function() {
                zk.close();
...
ServiceDiscoveryClient.prototype.getServices = function getServices(appName, cb) {
    ...
    zk.once('connected',function(err) {
        ...
        zk.getChildren(path, false, function(err, children) {
            if (err) {
                rc = err.getCode();
                console.error("zk error: %d, %s, %s", rc, err, path);
                if (rc === -101) {
                    // no children, just return empty list
                    cb(null, []);
                } else {
    ...
    zk.once('connected',function(err) {
        ...
        zk.exists(path, function(err, path) {
            if (err) {
                ...
                if (path){
                    console.log("exists for path: %s", path);
                    cb(null, true);
                } else {
                    cb(null, false);
                }
...
ServiceDiscoveryClient.prototype.registerEndpoint = function registerEndpoint(appName, serviceName, point, cb) {
    ...
    console.log("Registering endpoint for app: %s, service: %s, endpoint: %s at path: %s", appName, serviceName, point, path);
    var zk = ZooKeeper.createClient(this.zookeeper_string,{sessionTimeout: 200000, debug_level: ZooKeeper.ZOO_LOG_LEVEL_WARN,host_order_deterministic: false });
    zk.once('connected',function(err) {
        if (err) throw err;
        console.log("zk session established, id=%s", zk.getSessionId());
        zk.mkdirp(path, function(err, success) {
            if (err) {
                console.error("registerEndpoint error: %s", err);
            }
            cb(err, success);
            process.nextTick(function() {
                zk.close();
...
ServiceDiscoveryClient.prototype.unregisterEndpoint = function unregisterEndpoint(appName, serviceName, point, cb) {
    ...
    var path = this.makeZKPath(NAMESPACE, appName, serviceName, ENDPOINTS_ZK_PATH, point);
    console.log("Unregistering endpoint for app: %s, service: %s, endpoint: %s at path: %s", appName, serviceName, point, path);
    var zk = ZooKeeper.createClient(this.zookeeper_string,{sessionTimeout: 200000, debug_level: ZooKeeper.ZOO_LOG_LEVEL_WARN,host_order_deterministic: false });
    zk.once('connected',function(err) {
        if (err) throw err;
        console.log("zk session established, id=%s", zk.getSessionId());
        zk.remove(path, -1, function(err) {
            if (err) {
                var rc = err.getCode();
                console.log("zk error: %d, %s", rc, err);
                ...
                console.log("delete node: %s, error: %s", path);
                cb(null, true);
                process.nextTick(function() {
                    zk.close();
                });
