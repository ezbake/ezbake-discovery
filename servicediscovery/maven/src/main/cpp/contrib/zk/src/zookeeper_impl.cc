...
addAuth(const std::string& scheme, const std::string& cert,
  ...
  int rc = zoo_add_auth(handle_, scheme.c_str(), cert.c_str(),
                        static_cast<int>(cert.size()), completion, (void*)context,
                        isSynchronous);
...
create(const std::string& path, const std::string& data,
  ...
  ReturnCode::type rc = zoo_acreate(handle_, path.c_str(), data.c_str(),
          static_cast<int>(data.size()), acl, mode, completion, (void*)context, isSynchronous);
  if (rc != ReturnCode::Ok) {
    delete context;
  }
  return (ReturnCode::type)rc;
...
set(const std::string& path, const std::string& data,
  ...
  int rc = zoo_aset(handle_, path.c_str(), data.c_str(),
          static_cast<int>(data.size()), version, completion, (void*)context, isSynchronous);
  return (ReturnCode::type)rc;
}
